...
class MXNEnvelop(GridLayout):
    ...
    def update_graph(self, eps_arr, M_arr, N_arr):
        ...
        for i in range(n):
            # print(arr)
            p = LinePlot(
                color=[0, 0, 0], points=[(-eps_arr[0][i], 0), (-eps_arr[1][i], h)])
            self.graphLeft.add_plot(p)
        p = LinePlot(color=[0, 0, 0], points=[(m, -n)
                                              for n, m in zip(N_arr, M_arr)])
    ...
    def calculation(self):
        ...
        eps_arr = self.get_strain_arrays(
            eps_cu, eps_ccu, eps_u_r, y_r, env_reinf_idx, n_eps)
        # get the m-n envelop
        ...
        for i in np.arange(len(eps_arr[0])):
            result = self.explorer.calculation(
                eps_arr[0][i], eps_arr[1][i], n_eps)
            N_arr[i] = result[0]
            M_arr[i] = result[1]
            print(
                'eps_lower: ' + str(eps_arr[0][i]) + ' eps_upper: ' + str(eps_arr[1][i]) + ' i: ' + str(i))
            print('N: ' + str(N_arr[i]))
            print('M: ' + str(M_arr[i]))

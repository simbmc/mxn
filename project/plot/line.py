...
class OwnLine(Plot):
    ...
    def __init__(self, **kwargs):
        super(OwnLine, self).__init__(**kwargs)
        self.bind(
            color=self.ask_draw, points=self.ask_draw)

    def create_drawings(self):
        self._image = Line(width=10)
        return [self._image]

    def draw(self, *args):
        super(OwnLine, self).draw(*args)
        ...
        #bl = (funcx(self.points[0]) - xmin) * ratiox + \
        #    size[0], (funcy(self.pos[1]) - ymin) * ratioy + size[1]
        image.points = [self.points[0], self.points[1]]
        #w = tr[0] - bl[0]
        #h = tr[1] - bl[1]
        #image.size = (w, h)
